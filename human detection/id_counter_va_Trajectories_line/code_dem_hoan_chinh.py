...
def estimate_person_height(camera_indices=[0], known_distance=2.0, known_height=1.7, stationary_threshold=50):
    # Model
    ...
    while True:
        for cap, window_name in zip(caps, window_names):
            ...
            for person in persons:
                ...
                cv2.putText(frame, label, (int(x), int(y) - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 255), 2)
                
                # Add debugging prints
                print(f"x: {x}, x1: {x1}, in_line: {in_line}, out_line: {out_line}")
